    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            if (ModelState.IsValid && WebSecurity.Login(model.UserName, model.Password, persistCookie: model.RememberMe))
            {
                ...
                //var users = Roles.GetUsersInRole("Agent");
                //var userInRole = Roles.IsUserInRole("Agent");
                var usersInRole = Roles.FindUsersInRole(Role.Referrer.ToString(), model.UserName).ToList();
                if (usersInRole.Any(user => user == model.UserName))
                {
                    return RedirectToLocal("/MembershipData");
                }
        ...
        public ActionResult LostPassword(LostPasswordModel model)
        {
           if (ModelState.IsValid)
           {
              ...
              if (user != null)
              {
                 ...
                 string from = "donotreply@smartdb.com";
        ...
        public ActionResult ResetPassword(ResetPasswordModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (model.ReturnToken.StartsWith("=3D"))
                {
                    token = model.ReturnToken.Substring(3);
                }
                //bool resetResponse = WebSecurity.ResetPassword(model.ReturnToken, model.Password);
                bool resetResponse = WebSecurity.ResetPassword(token, model.Password);
                if (resetResponse)
                {
